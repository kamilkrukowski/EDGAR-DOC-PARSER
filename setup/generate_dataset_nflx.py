...
def _generate_dataset(features: pd.DataFrame, doc, tikr ):
    trainset =[]
...
def generate_dataset_nflx():
    ...
    for tikr in ["nflx"]:
        ...
        for doc in tqdm(annotated_docs, desc=f"Processing {tikr}", leave=False):
            ...
            trainset +=_generate_dataset(features, doc, tikr )
    return trainset    
...
trainset = generate_dataset_nflx()
